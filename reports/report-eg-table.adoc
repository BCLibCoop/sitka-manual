...
_ILS User_ (aka Patron or User): The table contains patron records. Patron names, phone numbers, email 
address, and registration date can all be found in this table. Follow the links to the table 
_Current Library Card_ to find a patron's current barcode, _Circulation_ to find the circulation history, 
_Home Library_, _Mailing Address_, _Physical Address_, and _Main Profile Group_, etc. to find more 
information about the patron.
 
_Item_ (aka Circulating Item): This table contains item records. Item barcode, creation date, active date, 
last edited date, last copy status change date, and price are in this table. For related information like 
call number, circulating library, circ modifier, status, shelving location, etc., you need to follow the 
links to the respective table to find them. For title information you need to follow the Call Number table to the 
...
_Bibliographic Record_: This table contains title information. To find the basic bibliographic 
information such as title, author, ISBN, etc., follow the link to Simple Records Extract.
 
_Circulation_: This table contains circulation records, including pre-catalogued item circulations. 
When an item is checked out, a circulation record is created. When an item is renewed, the existing 
circulation record is closed and another circulation record is created. Below are some important 
...
_Non-catalogued Circulation_: This table contains non-catalogued circulations. When a non-catalogued checkout is recorded, a record is created in this table. 
Non-catalogued item category can be found in the linked Non-Cat Item Type table.
 
_In-house Use_: This table contains in-house use records for catalogued items.
 
*Non-catalogued In-house Use:*  This table contains the non-catalogued item in-house use records. Follow the link to Item 
Type to find the non-catalogued item category.
 
_Copy Transit_:  When an item is sent in transit, regardless of whether it is going back to its circulating 
...
_Hold Transit_: When an item is sent in transit to fill a hold, a hold transit record is created in this table 
...
_Hold Request_: When a hold is placed, a hold record is created in this table. You may find the hold receiver's 
...
[NOTE]
======
Target Object ID is shown as a link. But there is no linked table in the Source pane. The value in this 
...
_Billing Line Item_: This table contains 
...
_Billable Transaction with Billing Location_: this table contains the summary records of billings and payments. 
...
_Payments_: ALL contains all payment records. When a lump sum of payment is made on the staff client, one 
...
_Payments: Brick-and-mortar_: contains all payments made at the circulation desk.
 
_Payments: Desk_: Cash/Check/Credit Card payment.
 
_Payments: Non-drawer Staff_: Forgive/Work/Goods/Patron Credit payments.
