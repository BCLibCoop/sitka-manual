...
Evergreen database any library staff needing access to the reporter must sign and submit the 
...
within the staff client] or xref:_viewing_output_via_the_email_link[via the link in the report completion 
email].

Any library staff needing to view report output, but not run reports, must have the secondary permission group
_Reporter View Output Permissions_ granted to their acount. See xref:_managing_secondary_permission_groups[]
