...
Inventory in Evergreen involves staff checking in all items in a specific area, using the Checkin Modifier Update Inventory,  and then running a report
to list all the items that were not scanned. When items are checked in the date, time, and workstation is recorded in
the Update Inventory  field of the item record. This field is used by the inventory reports to determine which items have been inventoried and which have not. Staff can also scan or upload item barcode files into the Item Status screen,  and batch change the Update Inventory field from the Actions menu.
...
+
. Enable Checkin Modifier *Update Inventory*.
+
. Check the items in.
