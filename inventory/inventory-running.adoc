...
to list all the items that were not scanned. When items are checked in the date and time is recorded in
the Update Inventory  field of the item record. This field is used by the inventory reports to determine which items have been inventoried and which have not. Staff can also scan or upload item barcode files into Item Status screen,  and batch change the Update Inventory field.
