...
values in these fields are updated when an item is inventoried by using the
*Update Inventory* action in _Item Status_, the *Update Inventory* checkin 
modifier at _Check In_, or enabling *Update Inventory* with _In-House Use_.
...
* Check In
+
image:images/inventory/update-inventory-item-record-4.png[scaledwidth="75%"]
+
...
different sections.
+
* Sitka_templates -> Collection -> Item List by Item Attributes -> xref:_shelving_location_items_with_selected_shelving_location[Shelving Location : 
...
Best practice is to ensure that the Check In, Item Status, and In-House Use screens are set to
display the call number, shelving location, and circulation modifier xref:_display_columns[columns].

As items are being scanned for inventory, staff should keep an eye out for the following:
...
Items found with one of the above issues should be pulled, so that staff with cataloguing 
...
At Check In, turn on the _Update Inventory_ checkin modifier on all computers where items may be checked
...
Enable Update Inventory with In-House Use
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

If your library tracks in-house use, check the box for **Update Inventory** on all computers where items may be 
in-housed while you are doing inventory. This ensures that any items in-housed will have their 
inventory date updated when they are scanned.

. In Evergreen, go to **Circulation → Record In-House Use**.
. Check the **Update Inventory** check box.
+
image:images/inventory/inventory-inhouse-checkbox-1.png[scaledwidth="75%"]
+
. A banner will display across the top of the screen to indicate that Update Inventory is enabled.
+
image:images/inventory/inventory-inhouse-checkbox-2.png[scaledwidth="75%"]
