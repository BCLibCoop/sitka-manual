...
Local system administrators can renew all staff accounts except local system administrators. 
...
[CAUTION]
=========
Staff accounts without a working location cannot sign into the Evergreen staff client. 
=========
...
Editing Staff Accounts via My Account
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
See xref:_resetting_staff_account_password[] for more ways a staff password can be reset.

[CAUTION]
=========
Updating your staff account in the public catalogue is the same as updating it in the staff client.
...
Editing Staff Accounts via the Staff Client
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Local system administrators can edit all staff accounts in the staff client the same as you 
would edit a patron, with the exception of local system administrator accounts. 

See xref:_resetting_staff_account_password[] for ways a staff password can be reset.
...
** Local system administrators can use the *Send Password Reset Link* button on the _Edit_ tab in the patron account in the staff client OR assign a new password to 
...
Support to have a temporary password assigned and then use the *Change* button in _My Account_.
...
Library staff must send a request to https://bc.libraries.coop/support/[Co-op Support] if they need to have this
...
See xref:_secondary_permission_groups[].
