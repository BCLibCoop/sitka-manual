...
Public Catalogue Library Settings
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

There are a number of library settings that can be used by local system administrators to customize 
the public catalogue.

See xref:_library_settings_editor[] for additional information on editing library settings.

Public Catalogue Messaging Settings
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

A temporary banner with text and links can be applied across the top of your public catalogue, and a custom message can be set to display on log in.

image::images/admin/opac-banner-customize-1.png[]

OPAC Alert Banner: Display
++++++++++++++++++++++++++
Can be set to "true" to display the banner or "false" to hide the banner.

OPAC Alert Banner: Type
+++++++++++++++++++++++
The banner has four display options which change the text colour:
...
[NOTE]
======
Coop support recommends using "danger" for the best contrast.
======

OPAC Alert Banner: Text
+++++++++++++++++++++++
Banner text can be formatted using HTML.
...
| Italic (HTML) | hello       | `$$<i>hello</i>$$`            | _hello_
| Underline (HTML) | hello       | `$$<u>hello</u>$$`            | [.underline]#hello#
| Link (HTML) | hello       | `$$<a href="https://bc.libraries.coop">hello</a>$$`            | https://bc.libraries.coop[hello]
| Line Break (HTML) | hello hello | `$$hello<br>hello$$`            | hello
+
hello
|===
...
OPAC Login Message
++++++++++++++++++
Sets a message to be displayed after logging in to My Account.

////
ADDITIONAL SETTINGS TO ADD POST-UPGRADE

Patron Related Settings
^^^^^^^^^^^^^^^^^^^^^^^
...
xref:_personal_information[pending address display]

xref:_request_a_library_card[link to for self check related settings]
...
* _uses phone as default pin_
* OPAC Inactivity Timeout (in seconds)
* OPAC: Number of staff client saved searches to display on left side of results 
and record details pages
...
Libraries that allow patrons to self register using the xref:_request_a_library_card[Request a Library Card] functionality
