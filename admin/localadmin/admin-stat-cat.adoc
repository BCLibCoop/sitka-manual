...
+
* *Required*: If yes is selected, a value must be assigned for the stat cat when cataloguing a new item. Selecting no means the stat cat is not required
* *OPAC Visible*: If yes is selected, the stat cat name and the selected value are displayed in 
the item table when looking at a record in the public catalogue. If no is selected, it will not display there.
* *Checkout Archive*: If yes is selected, the stat cat and its value are kept in aged 
circulation records. If no is selected, it will not be.
