...
* *Required*: If yes is selected, a value must be assigned when creating a new account or saving an existing account. Selecting no means the stat cat is not required.
* *OPAC Visible*: If yes is selected, the stat cat name and the selected value are displayed in the patron’s account in the public catalogue. If no is selected, it will not be.
* *User Summary*: If yes is selected, the stat cat and value display in the patron summary within 
the patron account. If no is selected, it will not be. (The stat cat field will also not be visible in the patron account if no value has been entered for the patron.)
* *Free Text*: If yes is selected, staff can type in a value rather than being restricted to a 
the list of predefined entries.
* *Checkout Archive*: If yes is selected, the category and its values will be archived. If no is selected, it will not be.
* *SIP Field* and *SIP Format* are not currently used by Sitka.
+
