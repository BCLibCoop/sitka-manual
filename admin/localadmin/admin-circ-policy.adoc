...
** Has the patron reached their maximum checked out, overdue, lost or fine amount?
* Item Status
** Is the item in a status that can be checked out?
* Item's Shelving Location
** Is circulate set to true or false for the shelving location?
* Item's Circulate flag
** If the circulate is set to true for the shelving location, is circulate set to true or false 
for the particular item? 	
* Circulation Policies for the check out library
** Which of the check out library's policies match the attributes of the item and the patron type 
it's being checked out to?
* Library Operating Hours for the check out library
** Is the library normally open on the day the item would be due?
* Closed Dates Editor for the check out library
** Is the closed for a holiday or other reason on the day the item would be due?
...
. Enter the item barcode into *Item Status* and go to the *Detail View*.
+
...
.. The most common reason for an item to not follow the expected policy is because it has the wrong 
...
. To apply the correct policy you will need to check the item in and then back out to the patron.
...
If you're unsure about what values are used for different policies you run the circulation policy report
