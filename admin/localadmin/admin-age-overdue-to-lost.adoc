...
[NOTE]
======
This feature does not support a date parameter. *ALL* overdue items by selected 
patron profiles at selected libraries will be marked Lost. It is mainly for school 
libraries' end of term process. To automatically mark items lost when they are overdue for x days, 
contact Co-op support.
======

.Age Overdue Items to Lost
. Go to *Administration* -> *Local Administration* -> *Age Overdue Circs to Lost*
...
[NOTE]
======
Please email Support if you would like this feature turned on for your library. 
Please indicate if you would like to send an automatic email when the 
items are marked lost.
======
