...
Academic libraries may wish to use hard due dates to truncate due dates at the end of a semester (so that borrowing times are shorter than normal) or 
allow particular material to go out for an entire semester (so that borrowing times are longer than normal).  
...
If you wish to add hard due dates, please 
...
the end of term (so the normal borrowing time for students is truncated to a hard due date), while faculty members would always get to check out items for the
entire semester (so the normal borrowing time for faculty is extended to a hard due date).
