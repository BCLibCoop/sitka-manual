...
Anyone with access to the staff client may view these settings, but only a staff with 
...
+
. Click *Apply Changes*. 
. Evergreen will use this information in when calculating due dates, overdue fines, and whether or not
...
Evergreen Addresses set in Organizational Units appear in patron 

. Enter or update the information on applicable subtab on the _Addresses_ tab.
...
Evergreen uses different addresses in some places.
...
libraries, and ON for Ontario libraries.  This fiels is used to generate the URLs to My Account link 
in email notices and the links will not be generated properly if the province is not entered correctly.
