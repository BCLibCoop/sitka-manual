...
library see xref:_notifications_action_triggers[] for instructions. 

Notifications are sent in two ways, depending on the notification:
...
out via email.  You can check to see whether or not a notification was generated.
...
Courtesy notifications are generated and sent via email ahead of an item being due.  
...
or a manual phone call when their hold is captured. 
...
xref:_updating_and_testing_email_addresses[test email messages] to confirm the 
