...
| xref:_age_hold_protect_rules[Age-based Hold Protection] | Allows libraries to restrict holds to be picked up at a range of libraries | 
...
| xref:_circulation_modifiers[Circulation Modifier] | An identifier used for applying circulation policies or for statistical purposes |A shared 
list of Sitka circulation modifiers displays in the dropdown list for all libraries. Each library selects 
circulation modifiers to use, and defines its own circulation policy for each modifier. 
...
may create system level shelving locations for all branches to share. To edit existing shelving locations or create new ones see
xref:_shelving_location_editor[]. 
| xref:_item_statistical_category_values[Statistical Categories] | Item statistical categories created by your library or 
federation. | Use the dropdown list to choose which organization's entries to display. To edit existing statistical categories
 or create new ones see xref:_statistical_categories_item_editor[]. 
| xref:_item_statuses[Item Status] | Current status of the item | By default a new item record is assigned a status of In Process. Some 
...
. In the Item Attributes section of the Holdings Editor, click on the attribute you wish to edit.
. From the drop down menu choose your attribute value and then click Apply.
+
image::images/cat/holdings/item-attribute-apply-1.png[scaledwidth="75%"]
+
. The attribute will be highlighted in green and the attribute label will be in bold.
+
image::images/cat/holdings/item-attribute-apply-2.png[scaledwidth="75%"]
+
. Apply values to all desired attributes and click *Apply All & Save* or "Apply All, Save & Exit*.
. The item record is updated with the selected attributes. 

[NOTE]
======
The value for an attribute can be cleared by clicking the *Clear* button for the particular attribute.

image::images/cat/holdings/item-attribute-apply-3.png[scaledwidth="75%"]
...
. In the Item Attributes section of the Holdings Editor, click *Alerts*.
+
image::images/cat/holdings/item-alert-1.png[scaledwidth="75%",alt="Item Alerts"]
+
. Choose an alert type from the drop down list.
. Enter the message in the _Alert Note_ field.
. If desired, check the box for *Temporary?*. This can be useful if the alert is only needed 
to resolve a particular issue.
. Click *Add New*. The alert will display as a pending item alert.
+
...
+
. The alert will appear when the item is checked out and/or checked in depending on 
the alert type chosen.
...
image::images/cat/holdings/item-alert-1.png[scaledwidth="75%",alt="Item Alerts"]
+
. Make the the necessary changes and click *Apply Changes*
+
image::images/cat/holdings/item-alert-4.png[scaledwidth="75%",alt="Item Alerts"]
+
. The Alerts attribute will indicate that there is a modified alert.
+
image::images/cat/holdings/item-alert-4b.png[scaledwidth="75%",alt="Item Alerts"]
+
. Click *Apply All, Save & Exit*.
+
. The updated alert will appear when the item is checked out and/or checked in depending on 
the alert type chosen.
...
. In the item attributes section of the Holdings Editor, click *Notes*.
+
