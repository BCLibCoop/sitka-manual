...
Records that have holds associated with them can be deleted. Evergreen will automatically 
xref:_cancelling_holds[cancel the holds] as part of the deletion process. 

[NOTE]
======
Co-op Support recommends libraries run a report to catch holds cancelled by Evergreen through the deletion process. The template 
_Monthly Cancelled Holds List with Cancelation Cause_ can be run as needed or set up as a monthly recurring report. 
These holds will show with a cancel reason of _Untargeted expiration_.
======

Deleting Records via the Staff Catalogue
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

If you import or create a MARC record you don't end up using, you can delete it directly from the *MARC Edit* tab 
...
If you import or create a MARC records you don't end up using, you can delete them in batch 
from a xref:_record_buckets[record bucket].
