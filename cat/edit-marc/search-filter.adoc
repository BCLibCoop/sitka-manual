...
The terminology used in the Search Filter and Format Icon tables to indicate characters and values of
MARC record fields are:

* *Type* is Leader position 06 - Type of record
* *Bib Level* is Leader position 07 - Bibliographic level
* *Content Type* is 336 subfield a - Content type term
* *Form* is 008 position 23 - Form of item , except when Type = e, f, or g then it's position 29
* *Media Type* is 337 subfield a - Media type term
...
* *Accessibility Features* - relies on the values in the 341 field. See 
https://docs.libraries.coop/policy/_search_filters.html#_accessibility_features[Accessibility Features].
...
| All Videos | Type = g
| Audio Device | Type = i *AND* Form = q
...
| Pictures | Type = k
| Preloaded Audio | (Type = i *AND* Form = q) *OR* ((Media Type = audio *OR* Content Type = spoken word) *AND* 
Format Icon = Book))
...
| Picture | Type = k
| Preloaded Audio | (Type = i *AND* Form = q) *OR* ((Media Type = audio *AND* Content Type = spoken word) *AND* 
Format Icon = Book))
