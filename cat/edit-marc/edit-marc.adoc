...
[TIP]
=====
You can set the displayed veiw as default by clicking *Set Default View*. Individual records will be loaded in this view when selected. 
=====
+
...
+
. To add or remove rows, or replace tags, click the *Help* button to find the keyboard shortcuts. Move the cursor to appropriate positon, then use the keyboard shortcut.
+
image::images/cat/marc-edit-5.png[]
+
. To work with the data in a tag simply click or Tab into required field.
. To edit indicators, click or Tab into required field and right click to view correct indicators and enter appropriate data.
+
