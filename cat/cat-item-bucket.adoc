...
There are three ways to create an item bucket. You can either create a bucket first, without accessing any copies, create an item bucket from a .csv file, or you can access an item record and choose to create the bucket from that view. We will demonstrate both methods here.

* xref:_item_bucket_create[Create an Item Bucket on Item Buckets view]
* xref:_item_bucket_file[Create an Item Bucket from File]
* xref:_item_bucket_record[Create an Item Bucket when Adding an Item Record to a Bucket]

Create an Item Bucket on Item Buckets view
++++++++++++++++++++++++++++++++++++++++++
[[_item_bucket_create]]

. Go to *Cataloging -> Item Buckets*.
...
Create an Item Bucket from File
+++++++++++++++++++++++++++++++
[[_item_bucket_file]]

You can upload a .csv file of multiple barcodes using the *Choose File* option.
...
image::images/cat/buckets/item-bucket-1.png[]
+
. Type in a name and some description, if needed.
+
. Click *Choose File*.
+
image::images/cat/buckets/item-bucket-file-1.png[]
+
. Follow the on-screen prompts to select your file.
+
.. If you want to include deleted barcodes, ensure the *Include deleted?* check box is checked.
+
. Click *Create Bucket*.
+
image::images/cat/buckets/item-bucket-file-2.png[]
+
. The newly created bucket is the active bucket in Bucket View.  Note that the bucket is numbered, 
and creator/owner identified.
+
image::images/cat/buckets/item-bucket-file-3.png[]
...
////
SECTION ON TRANSFERRING ITEMS TO BE ADDED

After the destination or target record has been marked as the target, navigate to the item bucket with items to be 
transferred. Then, select the items in the bucket that should be transferred, and from the *Actions* menu select 
*Transfer Selected Items to Marked Call Number*.
////
