...
. Alert can be made tempoprary by selecting the *Temporary* checkbox when adding the alert. A *Clear* button shows up when the alert is displayed. You may click it to delete the alert. Below is a checkin alert reminding staff to print spine label for the item. Click *Clear*, then *OK/Continue*.
+
...
Copy Statistical Category, aka Copy Stat Cat, is another piece of copy level information that may be added to copy records. As the namae suggests, it may be used to facilatate reporting categories, if not avaialabe in copy records.  Libraries need to create their own stat cat. Refer to xref:uio[].

To add copy stat cat, go to copy editor. Under *Statistical Category* column, choose an entry from the appropirate stat cat dropdown list. The value is added when the copy record is saved.
