...
The appropriate title information displayed on OPAC relies on correct coding. For example, format icons for text, moving pictures and sound relies on  MARC coding in the leader, and 007 and 008 fields, as do OPAC search filters, such as publication date, item type, or target audience. Bibliographic records matching and de-duplicating also rely on correct MARC coding and consistency in use of and contents in particular MARC tags. The ability to edit and manage MARC records is very important to maintaining database integrity.
