...
+
* Make sure both *Show empty volumes* and *Show empty libraries* checkboxes are selected to see libraries without copies on the library list.
=====
+
...
Adding Multiple Copies/Volumes to One Library
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
