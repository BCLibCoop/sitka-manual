...
. Get to the queue (better instruction needed)
. Click on the value in the Matches column for a record.
. On the Record Matches tab click on the Record ID to view the match(es). You may wish
to set your staff catalogue default tab to View MARC.
. Click the browser back button to return to your queue.
. Click on the Queued Record MARC tab.  Confirm the existing match from the catalogue is a match
for your incoming record.
. If there are multiple matches select the line of the one you want to match with. 
. If the existing match is a false match do not select it. 
. Click *Return to Queue*.
. Repeat for all records in the queue.
. An asterisk will appear in the Matches column beside each record for which a match
has been selected.
...
. Choose your Merge Profile - MORE DETAILS HERE
. Click *Import*.
