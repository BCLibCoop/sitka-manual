...
and a http://docs.libraries.coop/policy/_the_bibliographic_record.html#_a_good_marc_record[good MARC record] will exceed the minimum requirements.
======
+
. Click *Save*.
. The record is created and will open up in your tab in the current default view.
. xref:_adding_holdings[Add your holdings] to the record.
