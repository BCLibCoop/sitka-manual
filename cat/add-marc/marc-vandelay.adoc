...
Queues are viewable to the login account only. They will remain on the server until they are deleted. You may delete a queue after importing is finished. Or you keep them for a short period, then delete it.
...
To delete queues, select the checkboxes in front of the queue name and then clicking *Delete Selected* at the bottom.