...
Queues are viewable to the login account only. They will remain on the server until they are deleted. You may delete a queue after importing is finished. Or you can keep them for a short period, then delete it.

To delete a queue, go to *Cataloguing -> MARC Batch Import/Export*. The default screen is Import Records. Click *Inspect Queue* tab to manage and view existing queues used in MARC Batch Import.
...
To delete queues, select the checkbox in front of the queue name and then clicking *Delete Selected* at the bottom.
