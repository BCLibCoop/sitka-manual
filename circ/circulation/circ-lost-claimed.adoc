...
[NOTE]
======
There is variation between interfaces with what specific wording is used for some of these actions. 
The most commonly used wording is used in the instructions here.
======
...
. Select the item and choose *Mark Claims Never Checked Out* from the *Actions* menu.
+
...
. The item is checked in and 
set to _Missing_. The item is no longer associated with the patron's account.
. The patron's _Claims Never Checked Out Count_ is automatically increased. This can be viewed in the patron summary
and the _Edit_ tab.
...
. Go to the _Items Out_ tab in the patron account.
. Select the item and choose *Mark Claims Returned* from the *Actions* menu.
+
...
+
. The patron's _Claims-returned Count_ is automatically increased. This can be viewed in the patron summary
and the _Edit_ tab.
+
[NOTE]
======
Some libraries prefer to use the status _Missing_ rather than _Claimed Returned_.  This can be set using the library setting 
...
item has been found.  If the item is found in the library staff may wish to adjust the patron's 
_Claims-returned Count_
accordingly on the _Edit_ tab.

Damaged
^^^^^^^
If a patron damages an item or it is found damaged in the library it can be marked as damaged at check in, 
from within a patron's account, from the Holds Pull List, from the Holds Shelf, or via the Item Status interface.

. On the appropriate interface select the item and choose *Mark Items Damaged* from the *Actions* menu.
+
...
from the Holds Pull List, from the Holds Shelf, or via the Item Status interface. Libraries may wish to 
use the _Discard/Weed_ status when items to be discarded need to be reviewed before being deleted.

. On the appropriate interface select the item and choose *Mark Item Discard/Weed* from the *Actions* menu.
+
...
If an item cannot be located, it can be marked as missing from the Holds Shelf, from the Holds Pull List,
or via the Item Status interface.

. On the appropriate interface select the item and choose *Mark Item Missing* from the *Actions* menu.
+
image:images/circ/mark-missing-1.png[]
+
. A pop-up appears where staff can confirm that they wish to mark the item by clicking *OK/Continue*.
. The item is set to _Missing_.
...
are never returned and the item is deleted.  Notes, alerts, and blocks related to the item must be manually removed from
...
. Go to the _Items Out_ tab in the patron account.
. Select the item and choose *Mark Lost (By Patron)* from the *Actions* menu.
+
...
the item and displays highlighted in pink. A processing fee is included if the library has a value in the 
...
See xref:_bills_and_payments[] for more information on managing bills for lost items.
