...
from within a patron's account, via the Item Status interface, via the Holds Pull List, or from the Holds Shelf.

. On the appropriate interface select the item and choose *Mark Item as Damaged* from the *Actions* menu.
+
...
. On the appropriate interface select the item click *Actions -> Mark Items Discard/Weed*, *Actions -> Mark -> Item as Discard/Weed*, or *Actions -> Mark Item Discard/Weed* (depending on interface).
+
...
If an item cannot be located, it can be marked as missing via the Item Status interface or from the Holds Shelf.

. On the appropriate interface select the item and click *Actions -> Mark -> Item as Missing* or *Actions -> Mark Item Missing* (depending on interface).
+
