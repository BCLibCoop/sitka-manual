...
When an item is scanned for check out Evergreen checks the following to determine if the item can be 
checked out to the patron and if so with what parameters:
...
* xref:_circulation_policies[Circulation Policies] for the check out library
