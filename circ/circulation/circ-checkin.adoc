...
* Capture Local Holds as Transits: Local holds will be given an _In transit_ status instead of _On holds shelf_. The intent 
is to stop the system from sending holds notifications before the item is ready to be placed on the holds shelf, and item 
will have a status of _In transit_ until checked in again. If you wish to simply delay notification and allow time for staff 
to process item to holds shelf, you may wish to use the library setting _Hold shelf status delay_ instead.
