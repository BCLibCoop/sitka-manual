...
.. If creating a new user bucket, a dialog box called _Create Bucket_ will appear where you can enter a bucket _Name_ and _Description_ and indicate with the check box whether the bucket is _Shareable_.  Click *Create Bucket*.
...
All permissions must be granted at the organizational unit that the workstation is registered to or higher, 
and are checked against the users' Home Library when a batch modification or deletion is executed.
////
...
Notes, alerts, and blocks can be added to accounts in a user bucket by clicking the *Batch Create Notes* button.

. Go to *Circulation -> User Buckets*.
. Click *Buckets* and select the bucket which has the accounts that you want to add notes to.
. Click *Batch Create Notes*.
** For more detail on the differences between patron notes and how to apply changes, see xref:_staff_generated_notes[].

[NOTE]
======
Every patron in a bucket has a note added to their account when notes are batch created this way. Unchecking individual patrons in a bucket's list has no effect (a note is added to every patron account in the user bucket, regardless).
======
...
To delete users refer to xref:_deleting_patron_accounts[]
...
[[_sharing_user_buckets]]
Sharing Buckets
^^^^^^^^^^^^^^^

If a User Bucket has been made _Shareable_, it can be retrieved via bucket ID by another staff account.  The ID for each bucket can be found at the end of the URL for the bucket.  For example, in the screenshot below, the bucket ID is 32.
