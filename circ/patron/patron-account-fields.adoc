...
The following fields are shown by default (that is, when the library setting has no value, the field it relates to is visible).

Setting any of these Show library settings to False will hide them from a patron account as well as the patron registration form. A False value for a Show library setting does not delete information which previously had been saved in the field, but the information in the field will not be visible until the related library setting is deleted or set to True.
