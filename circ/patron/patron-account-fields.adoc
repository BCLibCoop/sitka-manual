...
|Collections: Exempt Check Box | No | Not currently used by Sitka.
...
|Valid Address? | N/A | Checked by default, this check box indicates that the address is valid
...
is being recorded for your patrons, and can cut down on unnecessary information being saved.

These customizations apply to both the xref:_registering_new_patrons[staff client patron registration form] and the patron self-registration form on your OPAC (if self-registration is enabled with xref:_library_settings_opac[the _Allow Patron Self-Registration_ library setting]).

These customizations also impact fields in the _Edit_ tab of a xref:_working_with_patron_accounts[patron account].

Show Fields
+++++++++++

These customizations impact fields in xref:_editing_patron_accounts[the Edit tab of a patron account], the _registering_new_patrons[staff client patron registration form], and xref:_request_a_library_card.html[patron self-registration] in your public catalogue (if you haveself-registration enabled).

When these library settings are not assigned a value, the related fields will be visible. A True value also means the related fields will be visible.

Setting any of these _Show_ library settings to False will hide the related field from the _Edit_ tab in a patron account, as well as the patron registration form.
...
[NOTE]
======
If a _Show_ library setting is set to False, information saved in the related field will not be visible until the library setting is deleted or set to True. Doing this does not delete the information from patron accounts which had values saved in the field.

For example, if values are saved in the Email Address field in patron accounts, and then the _Show email field on patron registration_ library setting is set to False, patron emails will not be visible or editable in patron accounts. Then, if the value for _Show email field on patron registration_ is deleted or set to True, the Email Address field will again appear in patron accounts, with the previously saved emails again displayed. (You will need to log out and back into the staff client for these changes to become visible.)
======
...
Libraries can display examples for how information should be input for certain fields. The applicable library settings are: 
