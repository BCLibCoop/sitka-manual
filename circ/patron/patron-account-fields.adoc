...
These customizations impact fields in xref:_editing_patron_accounts[the Edit tab of a patron account], the xref:_registering_new_patrons[staff client patron registration form], and xref:_request_a_library_card[patron self-registration] in your public catalogue (if you have self-registration enabled).
