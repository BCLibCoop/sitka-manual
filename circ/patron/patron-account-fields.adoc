...
apply to both the xref:_registering_new_patrons[staff client patron registration form] and the patron self-registration form on your OPAC (if self-registration is enabled with xref:_library_settings_opac[the _Allow Patron Self-Registration_ library setting]). These customizations also impact fields in the _Edit_ tab of a xref:_working_with_patron_accounts[patron account].
