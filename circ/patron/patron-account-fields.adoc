...
|Parent/Guardian Email Address | No | Can be used to include the parent or guardian's email address for juvenile patrons. *NOTE NOTE NOTE: THIS MUST BE UPDATED WITH INFO DEPENDING ON WHETHER OR NOT ACTION TRIGGERS CAN BE UPDATED TO INCLUDE THIS INFORMATION.* At this time, our A/T do not send updates to this address; see RT 132843. Either we say something like "this field is just a note, emails are not sent here" or, "this field can be set to also send emails to this address".
...
When the following library settings have no value, the related fields are visible.

Setting any of these _Show_ library settings to False will hide the related field from the Edit tab in a patron account, as well as the patron registration form.
...
The information saved in a field which becomes hidden if the related the _Show_ library setting is set to False will not be visible until the library setting is deleted or set to True. Doing this does not delete the information from patron accounts which had values saved in the field.

For example, if values are saved in the Email Address field in patron accounts, and then the _Show_ library setting for the email field is set to False, patron emails will not be visible or editable in patron accounts. Then, if the _Show_ library setting for email is deleted or set to True, the Email Address field will again appear in patron accounts, with the previously saved emails again displayed. (You will need to log out and back into the staff client for these changes to become visible.)
