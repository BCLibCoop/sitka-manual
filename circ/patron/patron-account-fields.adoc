...
These customizations impact fields in xref:_editing_patron_accounts[the Edit tab of a patron account], the xref:_registering_new_patrons[staff client patron registration form], and xref:_request_a_library_card.html[patron self-registration] in your public catalogue (if you have self-registration enabled).
...
When these library settings are not assigned a value, the related fields will be visible. A True value also means the related fields will be visible. The applicable library settings are:
