...
does not store the photos. This field is hidden by default and additional permissions are required to use this field. Please contact 
...
|Hold is behind Circ Desk Check Box | No | For libraries with a public holds shelf this can be used to 
indicate when a hold has not been put out publicly. The value of this check box displays in the Patron Summary. 
...
The patron account form can display *Required Fields*, *Suggested Fields*, or *All Fields*. 

image:images/circ/patron-fields-1.png[]

By default *All Fields* is selected.  A local system administrator at your library can set the default to 
*Suggested Fields* using the library setting _Default showing suggested patron registration fields_.
...
These customizations apply to the xref:_editing_patron_accounts[_Edit_ tab of a patron account], 
the xref:_registering_new_patrons[staff client patron registration form], and the xref:_request_a_library_card[patron self-registration].
...
[NOTE]
======
The _Email Address_ and _Default Phone Number_ fields are always required when
...
the account. The applicable library settings are:
...
[TIP]
=====
Setting any of the library settings that begin with _Show_ to FALSE will hide the applicable field so it does not
show at all in the patron account form regardless of whether *Required Fields*, *Suggested Fields*, or *All Fields* is selected.
=====
