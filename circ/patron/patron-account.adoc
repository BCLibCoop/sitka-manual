...
* Last Updated - Last time the account was updated via the _Edit_ tab.
* Create Date - Date on which the account was originally created.
* Expire Date - Date on which the account will expire.  This can be updated via the _Edit_ tab.
...
The _Check Out_ tab allows library staff to check items out to patrons.  

[NOTE]
======
If the patron's account has been retrieved with an inactive patron barcode the check out field
is grayed out and the patron will not be able to check out items.
======

For more information on the _Check Out_ tab see xref:_check_out[].
...
and xref:_checking_for_notifications[showing triggered events for notifications sent to patrons] about particular items.
...
The _Open Hold Requests_ tab shows holds that are waiting to be filled or are ready to be picked up by the patron.
...
The _Recently Canceled Holds_ tab shows holds that have been cancelled.  Holds can be cancelled by the patron, by staff, or by
Evergreen.
...
For information on the _Bills_ tab see xref:_bills_and_payments[].

Notes Tab
^^^^^^^^^

For information on the _Notes_ tab see xref:_patron_notes[].

Edit Tab
^^^^^^^^

For information on the _Edit_ tab see xref:_editing_patron_accounts[].
...
|Hold Groups | View any hold groups the patron is part of. For more information on hold groups
 see xref:_managing_hold_groups[].
...
in the same tab with the patron's barcode pre-filled.
|Completely Purge Account | Not used by Sitka. See xref:_deleting_patron_accounts[].
|===
