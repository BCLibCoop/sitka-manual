...
or deliberately when items on the xref:_holds_pull_list[Holds Pull List] are pulled and checked in.  
...
. When a hold is captured at xref:_check_in[check in] a hold slip pop-up will appear, unless 
...
[NOTE]
======
Libraries with self pickup hold shelfs can xref:_customize_receipt_templates[customize the hold slip] to limit the amount of patron 
