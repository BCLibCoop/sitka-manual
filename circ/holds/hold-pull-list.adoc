...
Items that are available on your shelf and have been requested to fill a hold will appear on the holds pull list. 
Staff can print the list, pull the items from the shelf, and then check them
...
. Go to *Circulation → Holds Pull List*.
...
+
. Print the pull list. See xref:_printing_and_downloading_the_holds_pull_list[] for 
...
Printing and Downloading the Holds Pull List
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Interlibrary Connect and the Holds Pull List
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
