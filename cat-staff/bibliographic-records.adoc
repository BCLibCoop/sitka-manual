...
The *Staff View* tab contains information from the MARC record presented in a human-readable format. Hyperlinks are to a relevant search. For example, if a record has an ISBN, the ISBN is hyperlinked to a numeric search for this ISBN.
...
The *Conjoined Items* tab allows users with appropriate permissions to manage conjoined items from the bibliographic record. 
...
The *Shelf Browse* tab displays tiles containing records for other items appearing before and after the selected record by call number.
