...
image::bibliographic_records/expand_vert.png[Screenshot of the grid options buttons with the down caret button highlighted, which is second from the right.]

To access more grid options, select the gear button. A menu opens.

image::bibliographic_records/grid_options_menu.png[Screenshot showing the gear button selected, as described above. The menu is described below.]
...
. Select _Apply Filter_. The filter menu closes and the filter is applied.
