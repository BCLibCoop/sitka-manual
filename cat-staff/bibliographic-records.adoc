...
With the record summary panel collapsed, the caret icon has inverted (is now pointing downwards). Click the *∨ caret icon* to expand the record summary panel.
...
The _Staff View_ tab contains information from the MARC record presented in a human-readable format. Hyperlinks are to a relevant search. For example, if a record has an ISBN, the ISBN hyperlink is to a numeric search for this ISBN.
