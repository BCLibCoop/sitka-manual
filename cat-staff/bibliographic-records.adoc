...
With the record summary panel collapsed, the caret icon has inverted. Click the *caret icon ∨* again to expand the record summary panel.
...
Clicking any hyperlink will run a relevant search. For example, if a record has the Subject _Birds_, the hyperlinked text goes to a subject search for Birds.
