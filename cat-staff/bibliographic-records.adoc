...
The _Staff View_ tab contains information from the MARC record presented in a human-readable format. Hyperlinks are to a relevant search. For example, if a record has an ISBN, the ISBN is hyperlinked to a numeric search for this ISBN.
...
The _Added Content_ tab appears when a library subscribes to a service that provides added content and has it set up in Evergreen.
...
TIP: For a more detailed view of holdings with the ability to use actions, see xref:_holdings_view[Holdings View].
...
Use the xref:_marc_view[MARC View] tab for a printer-friendly version of the MARC record.
