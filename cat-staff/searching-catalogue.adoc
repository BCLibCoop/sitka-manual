...
_Format_: Make a selection from the _Format_ dropdown menu to specify what type of item you are seeking. Some options for format include: All Books, Music (CD, cassette), All Videos, Braille, Books (large print), and Software/Video Games.
...
You can only search one tag or subfield per row. To add additional rows, select the _plus sign_ (+) to the right of the search row. To remove rows, select the _minus sign_ (-) to the right of the search row.
