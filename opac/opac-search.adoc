...
simultaneously. If you want to combine more than three search options, use the *Add Search Terms* button to
...
[[_public_catalogue_search_filters]]
Search Filters
^^^^^^^^^^^^^^
...
[TIP]
=====
With the exception of Shelving Location, the search filters rely on 
accurate coding in the Leader, 007, 008, 336, and 341 fields. Please see the search filter 
and icon specifications for Sitka's Evergreen in xref:_search_filters_and_format_icons[].
=====
...
When the _Limit to Available Items_ checkbox is checked search results are limited based on an item’s current
...
[[_public_catalogue_numeric_search]]
Numeric Search
~~~~~~~~~~~~~~
...
[[_public_catalogue_expert_search]]
Expert Search
~~~~~~~~~~~~~
...
. Enter the three-digit tag number, the subfield (if relevant), and the value or text that corresponds to the tag. For example, to search by publisher name, enter 264 b Random House. 
+
.. To search several tags simultaneously, use the *Add Search Terms* button.
+
...
[[_public_catalogue_browse_the_catalogue]]
...
[[_public_catalogue_search_results]]
Search Results
~~~~~~~~~~~~~~

The search results are a list of relevant works from the catalogue. If there are many results, they are
divided into several pages. At the top of the list, you can see the total number of results. You can go back and
forth between the pages by clicking the Next or Previous at the top or bottom of the list or click the page number to go to that page directly.

Your search terms will be highlighted in both the search results and title details screens. The
checkbox *Enable Highlighting* enables highlighted search terms, and unchecking it turns highlighting off.
...
Above the results list there are _Limit to Available Items_, _Group By Formats and Editions_,
 and _Exclude Electronic Resources_ checkboxes. Checking
_Limit to Available Items_ will filter out those titles with no available copies in the library or libraries at
...
You may update your search results by using the facets. Selecting one of these facet links on the left
side of search results page narrows down the search results to your selected term. You may
also update your search by clicking the *Update Search Results* button at the top of the search
results. Updating a search in this way allows you to add search filters such as Publication Date, Format,
...
* Click the library name (in the screenshot example, *Maple Library*) to see details about the library, such as address and open hours.
* Click *Accessibility Features* to see accessibility information, if available; for more information, see Material Specific Cataloguing Guidelines about the https://docs.libraries.coop/policy/_accessibility_features_tab.html[Accessibility Features tab].
