...
+
image::images/opac/opac-holds-notification-options-1.png[scaledwidth="75%",alt="Holds"]
+
.. If no email is set, one can be set by clicking *Add an email address*. This will open the _Update Email_ screen. Enter your password and email, then click *Save Changes*.
+
.. If no phone number is set, check the check box and enter a phone number.
+
image::images/opac/opac-holds-3.png[scaledwidth="75%",alt="Holds"]
+
. You can suspend the hold but have it activate on a specific date, keeping your place in the queue.
+
. You can also set a date on which the hold will be cancelled.
+
...
*Problem: The patron has reached the maximum number of holds* - This indicates the patron has reached
