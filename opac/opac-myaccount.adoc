...
_Other Settings_ is where you configure settings around search results, history,
...
Hold history can be enabled on this screen.  The holds history is not retroactive;
...
Check Out history can be enabled on this screen. Check out history is not retroactive; it takes effect once you choose to retain your history.
...
[[_items_checked_out_public_catalogue]]
Items Checked Out
~~~~~~~~~~~~~~~~~
...
Check out history will only display if it has been enabled in your account's
xref:_other_settings[_Other Settings_].  History
...
[[_public_catalogue_my_account_holds]]
Holds
~~~~~
...
or cancel the selected holds. The suspend, activate, and cancel buttons can also be used in the
_Edit_ column.
You may set an activation date when you suspend your hold
...
suspended, and what your position is in the hold queue. You can use the checkbox labelled
"Show only items available for pickup" to toggle showing all items or only those available for pickup.
You can also refresh the list of items by clicking on *Refresh holds list*.

Your position in the hold queue is indicated by the first number in the _Status_ column. For example
...
Holds history will only display if it has been enabled in your account's 
xref:_other_settings[_Other Settings_].

[[_public_catalogue_hold_groups]]
Hold Groups
^^^^^^^^^^^
(((Hold Groups)))
...
In _My Account_ click *Charges -> Current Charges* to display any outstanding charges.
