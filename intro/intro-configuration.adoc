...
the original workstation's name or you will need to reset all your grid settings across
 the staff client.
=====
...
grid will revert to its previous settings the next time it is viewed unless the grid 
settings are saved.
...
Depending on the data in the field the filters will either allow you to filter on yes/no/any
 or the text values.
